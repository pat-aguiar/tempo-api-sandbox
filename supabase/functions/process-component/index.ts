...
Deno.serve(async (req) => {
  try {
    ...
    console.log(`[ID: ${id}] Initializing Claude 4.6 for OpenAPI Generation...`)
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { 
            role: 'user', 
            content: `Generate a JSON OpenAPI 3.0.0 spec for the following component: \n\n${component_code}` 
          }
        ],
      }),
    })
    ...
    const rawContent = aiResult.content[0].text.trim()

    // 4. Validate and Parse JSON
    let generatedSpec;
    try {
      generatedSpec = JSON.parse(rawContent)
    } catch (_parseError) {
